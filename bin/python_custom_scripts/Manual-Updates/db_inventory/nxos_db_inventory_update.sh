...
## truncate the NXOS DB
python /usr/DCDP/bin/python_custom_scripts/Manual-Updates/db_inventory/NXOS/Truncate_NXOS_DB-Manual.py /usr/DCDP/bin/python_custom_scripts/Manual-Updates/db_inventory/NXOS/db_nxos_connection.txt 

## REPOPULATE THE NXOS inventory from live network discovery
python /usr/DCDP/bin/python_custom_scripts/Manual-Updates/db_inventory/NXOS/NXOS_Inventory.py /usr/DCDP/bin/python_custom_scripts/Manual-Updates/db_inventory/NXOS/db_nxos_connection.txt 

# kill all python thread processes to cleanup as a safety-net
ps -fLu root | grep python | awk {'print $2'} | xargs kill -9

### create new backup SQL DB to DataCenter, then regenerate new NXOS xml file.....
echo "###########################################################################################"
echo " Generating new NXOS xml file and backing up the NXOS SQL DB to $DataCenter."
echo "###########################################################################################"

 user="root"
 password="dcdp"
 host="localhost"
 db_name="NXOS_INVENTORY"
 db_table="Cisco_NXOS_Inventory"
# Other options
