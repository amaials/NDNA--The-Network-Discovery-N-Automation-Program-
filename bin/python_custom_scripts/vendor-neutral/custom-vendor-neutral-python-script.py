...
This is your custom python script. It can be used to thread to devices and either 
program the devices or pull ANY information from the devices. 

Output files for each node will be written to the /usr/DCDP/configs directory using separate files for each device.

This is the sript to use across both NXOS or IOS devices, e.g. it points to the "Good-IPs.txt" file 

There will be a separate IOS and NXOS specific script, which points to those specific IP files

Just change the commands in the ssh_commands.txt file, just as if you were at the CLI

NOTE- You can just as easily point to ANY IP file and use this custom script to program or pull information 

from ANYTHING you have ssh access to, e.g. 

Juniper, Arista, ESXi, etc...
...
print "            /_____/   "
print "            "
print "       For IOS, NX-OS, or Any IP Node, e.g. Cisco, Juniper, etc."
print "   Just change your backend ssh_custom_vendor_neutral_commands.txt file" 
print "   and the vendor-neutral-IPs.txt file to the IPs you want to connect to"
print ""   

class Logger(object):
    def __init__(self):
        self.terminal = sys.stdout
        self.log = open("/usr/DCDP/logs/custom_vendor-neutral_python_script.log", "w")
...
sys.stderr = open("/usr/DCDP/logs/custom_vendor-neutral_python_script_err.log", "w")
...
def cmd_is_valid():
    ...
    while True:
        cmd_file = "ssh_custom_vendor_neutral_commands.txt"
        
        #Changing output messages
        if os.path.isfile(cmd_file) == True:
            print "\n* Attempting to connect to device(s)...\n"
            print "\n* Output files will be written to /usr/vendor_neutral/configs for all device(s)...\n" 
...
iplist = open('/usr/DCDP/bin/python_custom_scripts/vendor-neutral/vendor-neutral-IPs.txt').readlines()
# remove return from file names, e.g. remove \n from list
iplist = map(lambda s: s.strip(), iplist)

def write_files(ip):
    file_name = '/usr/vendor_neutral/configs/' + ip + '_' + current_time + '_vendor-neutral_custom.txt'
    fo = open(file_name, "w")
