...
print "            /_____/   "
print "            "
print "                    For Enterprise Wide Switches"
print ""
print "           Just change your backend ssh_custom_enterprise-switches_commands.txt file" 
print "  And the /usr/enterprise-wide-switches/enterprise-wide-switches-IPs.txt file to the IPs you want to connect to"
print ""   
...
class Logger(object):
    def __init__(self):
        ...
        self.log = open("/usr/DCDP/logs/custom_enterprise-switches_python_script.log", "w")
...
sys.stderr = open("/usr/DCDP/logs/custom_enterprise-switches_python_script_err.log", "w")
...
def cmd_is_valid():
    ...
    while True:
        cmd_file = "ssh_custom_enterprise-switches_commands.txt"
        #Changing output messages
        if os.path.isfile(cmd_file) == True:
            print "\n* Attempting to connect to device(s)...\n"
            print "\n* Output files will be written to /usr/enterprise-wide-switches/configs for all device(s)...\n" 
...
iplist = open('/usr/enterprise-wide-switches/enterprise-wide-switches-IPs.txt').readlines()
# remove return from file names, e.g. remove \n from list
iplist = map(lambda s: s.strip(), iplist)

def write_files(ip):
    file_name = '/usr/enterprise-wide-switches/configs/' + ip + '_' + current_time + '_enterprise-switches_custom.txt'
    fo = open(file_name, "w")
