...
This is your custom NXOS python script. It can be used to thread to devices and either 
...
This is the sript to use for NXOS devices, e.g. it points to the "NX-OS-IPs.txt" file 

Just change the commands in the ssh_nxos_commands.txt file, just as if you were at the CLI
...
class Logger(object):
    def __init__(self):
        ...
        self.log = open("/usr/DCDP/logs/custom_nxos_python_script.log", "w")
...
sys.stderr = open("/usr/DCDP/logs/custom_nxos_python_script_err.log", "w")
...
def cmd_is_valid():
    ...
    while True:
        cmd_file = "ssh_custom_nxos_commands.txt"
...
#############################################################
        ...
        if re.search(r"% Invalid command at", router_output):
            print "* There was at least one NXOS syntax error on device %s" % ip
...
def write_files(ip):
    file_name = '/usr/DataCenters/%s/DCDP/configs/' % DataCenter + ip + '_' + current_time + '_nxos_custom.txt'
    fo = open(file_name, "w")
