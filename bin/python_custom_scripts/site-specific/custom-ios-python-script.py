...
This is your custom IOS python script. It can be used to thread to devices and either 
...
This is the sript to use for IOS specific commands, e.g. commands that would not be supported on NXOS devices, 

e.g. it points to the "IOS-IPs.txt" file 

Just change the commands in the ssh_ios_commands.txt file, just as if you were at the CLI
...
class Logger(object):
    def __init__(self):
        ...
        self.log = open("/usr/DCDP/logs/custom_ios_python_script.log", "w")
...
sys.stderr = open("/usr/DCDP/logs/custom_ios_python_script_err.log", "w")
...
def cmd_is_valid():
    ...
    while True:
        cmd_file = "ssh_custom_ios_commands.txt"
...
iplist = open('/usr/DataCenters/%s/DCDP/good-IPs/IOS-IPs.txt' % DataCenter).readlines()
# remove return from file names, e.g. remove \n from list
iplist = map(lambda s: s.strip(), iplist)

def write_files(ip):
    file_name = '/usr/DataCenters/%s/DCDP/configs/' % DataCenter + ip + '_' + current_time + '_ios_custom.txt'
    fo = open(file_name, "w")
