...
print "            /_____/   "
print "            "
print "                    For IOS and NX-OS Devices:    "
print "     Just change your backend ssh_custom_ios_nxos_commands.txt file" 
print ""   
print "###############################################################################################" 
...
class Logger(object):
    def __init__(self):
        ...
        self.log = open("/usr/DCDP/logs/custom_ios_nxos_python_script.log", "w")
...
sys.stderr = open("/usr/DCDP/logs/custom_ios_nxos_python_script_err.log", "w")
...
def cmd_is_valid():
    ...
    while True:
        cmd_file = "ssh_custom_ios_nxos_commands.txt"
...
iplist = open('/usr/DataCenters/%s/DCDP/good-IPs/Good-IPs.txt' % DataCenter).readlines()
# remove return from file names, e.g. remove \n from list
iplist = map(lambda s: s.strip(), iplist)

def write_files(ip):

    file_name = '/usr/DataCenters/%s/DCDP/configs/' % DataCenter + ip + '_' + current_time + '_ios_nxos_custom.txt'
    fo = open(file_name, "w")
