...
### Program will be run from /usr/NTC/cacti

echo ""
echo ""
if (dialog --title "Welcome to The NDNA-to-Cacti-Integration" --yesno "         Choose Yes to Begin, or No to Exit." 10 60) then
...
echo ""
echo "You have the option of only importing L3 devices into Cacti (IOS only)...."
echo ""
echo "You will get a prompt allowing you to confirm if you want to do this....."
echo ""
echo "If you answer no, the regular program will continue, importing all devices from the DataCenter you specify....."
echo ""
sleep 10
read -r -p "Do you want to only build in L3-Devices? Answer yes if so, or No to build in ALL devices: [y/N] " response
echo ""
...
then
    ...
    echo "Continuing to Run Normal NDNA-to-Cacti-Integration import (All L2-L3 Devices)...."
fi
...
exec 2> >(tee /usr/NTC/cacti/NTC-date.log)
...
# copy over IOS CSV file from DataCenter
cp /usr/DataCenters/$DataCenter/DCDP/hostname-to-IPs/hostname-to-IPs-IOS.csv /usr/NTC/cacti

# SEND TO NEW FILE WITH variable DC name in the filename
cat hostname-to-IPs-IOS.csv > NDNA-$DataCenter-to-CACTI.csv
...
echo "Building Devices into Remote Cacti Server....Standby"
echo "#########################################################"
echo ""
...
echo "#################################################################################"
echo ""
echo "    Devices for the NDNA DataCenter $DataCenter have been built into cacti"
echo ""
