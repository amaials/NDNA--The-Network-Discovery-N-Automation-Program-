...
date=$(date +"%b-%d-%y")
echo "Building Enterprise Wide IP list (For ALL L2 Devices)"
echo ""
find /usr/DataCenters/$Company*/DCDP/* -name "L2-*-IPs.txt" -exec cat {} \; > /usr/enterprise-wide-switches/Created-IP-Lists/$Company-enterprise-wide-ios-nxos-switches-$date.txt
echo "Enterprise Wide IP list for $Company (For ALL L2 Devices) Has Completed..."
echo ""