...
date=$(date +"%b-%d-%y")
echo "Building Enterprise Wide IOS IP list (For L2 Devices)"
echo ""
find /usr/DataCenters/$Company*/DCDP/* -name "L2-IOS-IPs.txt" -exec cat {} \; > /usr/enterprise-wide-switches/Created-IP-Lists/$Company-enterprise-wide-ios-switches-$date.txt
echo "Enterprise Wide IOS IP list for $Company (For L2 Devices) Has Completed..."
echo ""