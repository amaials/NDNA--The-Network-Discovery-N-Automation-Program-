...
This is your VNAD Arista python script. It can be used to thread to devices and either 
...
class Logger(object):
    def __init__(self):
        ...
        self.log = open("/usr/VNAD/logs/%s/arista_python_script.log" % DataCenter, "w")
...
sys.stderr = open("/usr/VNAD/logs/%s/arista_python_script_err.log" % DataCenter, "w")
...
def cmd_is_valid():
    ...
    while True:
        cmd_file = "ssh_arista_commands.txt"
...
def write_files(ip):
    file_name = '/usr/VNAD-LEs/%s/VNAD/configs/' % DataCenter + ip + '_' + current_time + '_arista.txt'
    fo = open(file_name, "w")
...
def write_files2(ip):
    file_name = '/usr/DataCenters/%s/VNAD/configs/' % DataCenter + ip + '_' + current_time + '_arista.txt'
    fo = open(file_name, "w")
    #Calling the SSH function
...
if (os.path.isdir(VNADDIR)):
    print "Calling SSH Connection to VNAD-LE Arista nodes..."
    print "\n* Output files will be written to /usr/VNAD-LEs/%s/VNAD/configs for all device(s)...\n" % DataCenter
    iplist = open('/usr/VNAD-LEs/%s/VNAD/IP-Lists/Arista/Arista_ip_file-py.txt' % DataCenter).readlines()
    ...
    print "Calling SSH Connection to Data-Center VNAD Arista nodes..."
    print "\n* Output files will be written to /usr/DataCenters/%s/VNAD/configs for all device(s)...\n" % DataCenter
    iplist2 = open('/usr/DataCenters/%s/VNAD/IP-Lists/Arista/Arista_ip_file-py.txt' % DataCenter).readlines()
    # remove return from file names, e.g. remove \n from list
