...
class Logger(object):
    def __init__(self):
        ...
        self.log = open("/usr/DCDP/logs/sh-cdp-nei-nxos.log", "w")
...
sys.stderr = open("/usr/DCDP/logs/sh-cdp-nei-nxos-ERR.log", 'w')
...
#############################################################
        ...
        if re.search(r"% Invalid command", router_output):
...
###################################################################
        ...
        print "* Check your logs and run Manual Update for NXOS cdp-neighbors file...."
...
def write_files(ip):
    file_name = '/usr/DCDP/tmp/' + ip + '_cdp-nei-nxos-ips.txt'
    fo = open(file_name, "w")
