...
#cat /usr/DCDP/logs/dcdp.log | grep '** Completed IOS' | awk '{print $NF}' > /usr/DCDP/good-IPs/IOS-IPs.txt
cat /usr/DCDP/hostname-to-IPs/hostname-to-IPs-IOS.csv | sed -e 's/_seed.*//' | sed -e 's/_seed//' | sed -e 's/,/ ,/' | awk {'print $1'} | sed -e 's/IP-Address//' | sed '/^\s*$/d' > /usr/DCDP/good-IPs/IOS-IPs.txt
cat /usr/DCDP/logs/dcdp.log | grep '** Completed WLC' | awk '{print $NF}' > /usr/DCDP/good-IPs/WLC-IPs.txt